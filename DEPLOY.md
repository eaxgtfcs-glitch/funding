...
```bash(Linux)
 cd funding;git pull https://github.com/eaxgtfcs-glitch/funding;docker stop funding;docker rm funding;docker build -t funding-app .;docker run -d   --name funding   --restart unless-stopped   --env-file .env   -v $(pwd)/data:/app/data   funding-app;docker logs -f funding
```
